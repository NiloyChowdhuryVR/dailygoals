...
export const VideoVaultView: React.FC<VideoVaultViewProps> = ({ onOpenAddModal }) => {
  const { savedResources, toggleResourceWatched, deleteResource } = useProgress();
  ...
  // Extract all unique tags across all resources
  ...
  }, [savedResources, searchQuery, selectedTag, selectedType, selectedStatus]);
  ...
  const handleToggleWatched = (e: React.MouseEvent, res: SavedResource) => {
    ...
    if (!res.isWatched) {
      ...
      confetti({
        ...
        spread: 60,
        origin: { x, y },
        colors: ['#10b981', '#3b82f6', '#8b5cf6', '#a855f7'],
  ...
  const watchLaterCount = savedResources.filter((r) => !r.isWatched).length;
  const watchedCount = savedResources.filter((r) => r.isWatched).length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800/80 pb-4">
        <div className="space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="px-2.5 py-0.5 rounded-full bg-purple-950 text-purple-300 text-xs font-mono font-bold border border-purple-800/60">
              {savedResources.length} Saved Item{savedResources.length !== 1 ? 's' : ''}
            </span>
          </div>

          <p className="text-xs sm:text-sm text-slate-400">
            Store video tutorials, deep-dive lectures, and playlists with custom notes and tags.
          </p>
        </div>
        ...
        <button
          ...
          className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 via-blue-600 to-teal-500 hover:from-purple-500 hover:to-teal-400 text-white font-extrabold text-xs sm:text-sm shadow-lg shadow-purple-600/20 transition-all active:scale-95 flex items-center justify-center gap-2 self-start md:self-auto shrink-0"
        >
      ...
      <div className="space-y-3 bg-dark-900/60 p-3.5 sm:p-4 rounded-2xl border border-slate-800/80 backdrop-blur-md">
        {/* Search Bar */}
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search saved videos, playlists, tags, or notes..."
            className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-dark-850 border border-slate-800 text-white text-xs sm:text-sm placeholder-slate-500 focus:outline-none focus:border-purple-500 transition-all font-mono"
          />
        </div>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all shrink-0 ${
                selectedStatus === 'all'
                  ...
                  : 'bg-dark-850 border border-slate-800 text-slate-400 hover:text-slate-200'
              }`}
            >
      ...
      {filteredResources.length > 0 ? (
            ...
            {filteredResources.map((res) => {
              ...
              return (
                <motion.div
                  ...
                  className={`relative rounded-2xl border p-4 backdrop-blur-md transition-all flex flex-col justify-between space-y-3.5 ${
                    res.isWatched
                ...
                >
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="relative rounded-xl overflow-hidden bg-dark-950 border border-slate-800/80 aspect-video group">
                      {thumbnail ? (
                        <img
                          src={thumbnail}
                          alt={res.title}
                          className={`w-full h-full object-cover transition-transform duration-300 group-hover:scale-105 ${
                            res.isWatched ? 'opacity-50 grayscale-[40%]' : 'opacity-85'
                          }`}
                        />
                      ) : (
                        <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-purple-950/40 to-blue-950/40 text-slate-400 p-4 text-center">
                          {res.type === 'playlist' ? (
                            <ListVideo className="w-10 h-10 text-blue-400 mb-1" />
                          ) : (
                      ...
                      >
                        <div className="w-12 h-12 rounded-full bg-blue-600 text-white flex items-center justify-center shadow-xl shadow-blue-500/30 transform group-hover:scale-110 transition-transform">
                          <Play className="w-6 h-6 fill-current translate-x-0.5" />
                        </div>
                      </a>

                      {/* Type Badge */}
                      <div className="absolute top-2 left-2 px-2.5 py-0.5 rounded-full bg-black/80 backdrop-blur-md text-[10px] font-extrabold font-mono border border-slate-700/60 flex items-center gap-1 text-white">
                        {res.type === 'playlist' ? (
                          <>
                      ...
                      {res.isWatched && (
                        <div className="absolute top-2 right-2 px-2.5 py-0.5 rounded-full bg-emerald-500 text-slate-950 text-[10px] font-extrabold font-mono shadow-lg flex items-center gap-1">
                    ...
                    {res.whyWatch && (
                      <div className="p-3 rounded-xl bg-dark-850/80 border border-slate-800/80 space-y-1 text-xs text-slate-300">
                        <div className="flex items-center gap-1 font-bold text-amber-400 text-[11px] font-mono uppercase">
                          <HelpCircle className="w-3.5 h-3.5" />
                          <span>Why to Watch This:</span>
                        </div>
                  ...
                  {/* Card Footer Actions */}
                  <div className="flex items-center justify-between gap-2 pt-3 border-t border-slate-800/60 shrink-0">
                    <button
                    ...
                    <div className="flex items-center gap-2">
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="px-3 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold transition-all flex items-center gap-1 shadow-md shadow-blue-600/20 active:scale-95"
                      >
                        <span>Watch</span>
        ...
        <motion.div
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          className="rounded-2xl border border-slate-800 bg-dark-850/40 p-8 md:p-12 text-center space-y-4 max-w-lg mx-auto"
        >
          <div className="w-16 h-16 mx-auto rounded-2xl bg-gradient-to-tr from-purple-600 to-blue-600 text-white flex items-center justify-center shadow-xl shadow-purple-500/20">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-slate-400 leading-relaxed">
              Save YouTube videos, concept tutorials, or entire playlists with notes on why to watch them.
            </p>
          </div>

          <button
            onClick={onOpenAddModal}
            className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-extrabold shadow-lg shadow-blue-500/20 transition-all flex items-center justify-center gap-2 mx-auto"
          >
            <Plus className="w-4 h-4" />
            <span>Save Your First Video or Playlist</span>
          </button>
        </motion.div>
      )}
    </div>
